...
from aquadapt_api_client_oficial import AquaAdvancedClient, load_bmb_list_from_file
import config
...
def ejemplo_consulta_individual():
    ...
    # Crear cliente sin parámetros (usa config.py automáticamente)
    ...
    speed = client.get_bomba_speed(bmb_id)
...
def ejemplo_monitoreo_multiples():
    ...
    client = AquaAdvancedClient(base_url=API_BASE_URL, verify_ssl=VERIFY_SSL)
    bmb_list = load_bmb_list_from_file(BMB_JSON_FILE)
...
def ejemplo_exportar_csv():
    ...
    try:
        ...
        print("pandas no está instalado. Instalado con: pip install pandas")
